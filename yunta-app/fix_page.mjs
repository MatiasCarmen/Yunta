...
// 1. Check if we already have getPendingPayments
if (!d.includes('getPendingPayments')) {
    d = d.replace(
...
// 2. Replace handleAddTransaction using regex if it wasn't replaced
const regex = /const handleAddTransaction = async \(txData: TransactionInput\) => \{[\s\S]*?\s+alert\('Error al registrar el pago'\);\n\s+\}\n\s+\};/;
...
const newHandler = `
  ...
  const handleRetrySync = async () => {
    ...
    // This will trigger window online manually or logic
  ...
  // Actualizado para usar hook de sincronizacion
...
if (d.match(regex)) {
    d = d.replace(regex, newHandler.trim());
}

// 3. Remove the older useJuntaSync implementation if it exists independently
d = d.replace(/\/\/\s*Offline sync hook\s*useJuntaSync\(junta\.id, async \(\) => \{\s*const updated = await getActiveJunta\(\);\s*if \(updated\) onUpdate\(updated\);\s*\}\);/, "");

// 4. Inject UI for pending / failed into return statement
const uiSnippet = `
...
console.log("Updated page handles successfully");
